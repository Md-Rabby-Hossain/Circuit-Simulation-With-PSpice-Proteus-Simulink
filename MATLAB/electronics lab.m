...
% semilog line in RED, line thickness = 1.5
semilogx(f,  g, '-o', 'Color','r', 'LineWidth',1.5, 'MarkerSize',6); hold on
semilogx(f1, g1, '-s', 'Color','k', 'LineWidth',1.5, 'MarkerSize',6); % black curve

% --- -3 dB horizontal lines ---
mid_rc = max(g); 
mid_dc = max(g1);
yline(mid_rc - 3, '--g', '-3 dB (RC)', 'LineWidth',2, 'LabelHorizontalAlignment','left'); % green
yline(mid_dc - 3, '--k', '-3 dB (DC)', 'LineWidth',2, 'LabelHorizontalAlignment','left'); % black

% --- vertical cutoff lines ---
if ~isnan(fL_rc)
    xline(fL_rc, '--g', ['fL(RC) = ' num2str(fL_rc,'%.2f') ' Hz'], 'LineWidth',2, 'LabelVerticalAlignment','bottom');
end
if ~isnan(fH_rc)
    xline(fH_rc, '--g', ['fH(RC) = ' num2str(fH_rc,'%.2f') ' Hz'], 'LineWidth',2, 'LabelVerticalAlignment','bottom');
end
if ~isnan(fL_dc)
    xline(fL_dc, '--k', ['fL(DC) = ' num2str(fL_dc,'%.2f') ' Hz'], 'LineWidth',2, 'LabelVerticalAlignment','top');
end
if ~isnan(fH_dc)
    xline(fH_dc, '--k', ['fH(DC) = ' num2str(fH_dc,'%.2f') ' Hz'], 'LineWidth',2, 'LabelVerticalAlignment','top');
end
